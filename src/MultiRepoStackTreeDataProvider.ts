  ...
  implements TreeDataProvider<AggregatedTreeItem>
{
  ...
  private readonly providerMap = new WeakMap<
    StackTreeData,
    StackTreeDataProvider
  >();
  ...
  removeRepository(repositoryPath: string): void {
    const index = this.repositories.findIndex(
      (r) => r.repositoryPath === repositoryPath
    );
    if (index === -1) {
      return;
    }
  ...
  async switchTo(branchOrStack: StackTreeItem | BranchTreeItem) {
    ...
    if (branchOrStack.type === "stack") {
      ...
      await this.providerFromElement(branchOrStack).switchTo(
  ...
  async getChildren(
    ...
    if (!element) {
      ...
      for (const { provider, repositoryName, repositoryPath } of this
        .repositories) {
        try {
